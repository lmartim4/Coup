...
from game_state import PendingDecision, PlayerStateView, GameStateView
...
class GameEngine:
    ...
    def __init__(self, players: list[Player]):
        ...
        self._phase_action      = None  # Influence aguardando seleção de alvo
        self._phase_defense     = None  # (target_idx, action): alvo decide como reagir
        self._phase_doubt_block = None  # dict: attacker, target, action, queue
        self._phase_challenge   = None  # dict: actor, action, queue
        self._phase_block_open  = None  # dict: actor, action, queue  (Ajuda Externa)
        self._phase_lose_inf    = None  # dict: target_idx, next_turn
        self._phase_reveal      = None  # dict: contexto completo do desafio
    ...
    def _emit_decision(self):
        ...
        if self._phase_reveal is not None:
            ...
            target_idx, action = self._phase_defense
            self.pending_decision = PendingDecision(
                player_index=target_idx,
                decision_type='defend',
                options=['block', 'doubt_action', 'accept'],
                context={
                    'action_name':   action.get_name(),
                    'block_card':    action.get_block_name(),
            ...
            self.pending_decision = PendingDecision(
                ...
                options=targets,
                context={'action_name': self._phase_action.get_name()},
            )
    ...
    def _on_pick_target(self, target_idx: int):
        ...
        if not action.is_challengeable():
            ...
            action.apply_cost(player)
            self._phase_defense = (target_idx, action)
        else:
    ...
    def _on_reveal(self, choice: str):
        ...
        if ctx == 'doubt_action':
            ...
            if has_card:
                print(f"{challenged.name} revelou {card_name}! Desafio falhou.")
                if action.causes_influence_loss():
                    # Assassino: desafiante perde 1 carta (desafio) + 1 carta (assassinação).
                    # Resultado sempre é eliminação — remove todas as cartas restantes.
                    t = self.players[target_idx]
                    for inf in list(t.influences):
                        t.revealed_influences.append(inf)
                        print(f"{t.name} perdeu: {inf.get_name()}")
                    t.influences.clear()
                    print(f"{t.name} foi eliminado!")
                    self.current_turn = next_turn
                else:
                    # Capitão: ação executada e desafiante perde 1 carta pelo desafio falho
